...
const Values = () => {
  ...
  const EditCate = async (i) => {
    setId(i);
    const { data } = await getValues(i);
    if (!data?.error) {
      let info = data?.results.value;
      console.log(info);
      setSelected(...selected, {
        ...
        attribute: info?.attribute?.name_en,
        attributeId: info?.attribute?._id,
      });
      reset2({
        edit_filterName_en: info?.filter_name_en,
        edit_filterName_ar: info?.filter_name_ar,
        edit_values_en: info?.name_en,
        edit_values_ar: info?.name_ar,
      });
    }
  };

  const getAllValues = async (date) => {
    const { data } = await AllNewValues({ year: date });
    const newRows = [];
    if (!data.error) {
      let values = data?.results?.values;
      values?.map((list, index) => {
        ...
        returnData.name_attribute = list?.attribute?.name_en;
  ...
  const valStatus = async (id) => {
    const { data } = await ValuesStatus(id);
    if (!data?.error) {
      getAllValues();
      Swal.fire({
  ...
  const ShipmentStatus = async (id) => {
    const { data } = await ValuesShipStatus(id);
    if (!data?.error) {
      getAllValues();
      Swal.fire({
  ...
  const GetAttributes = async (id) => {
    const { data } = await AttributesBySubSubCate({ subSubCategoryId: id });
    if (!data.error) {
      setAttributes(data?.results?.attributes);
    }
  };
  ...
  const onSubmit = async (data) => {
    ...
    if (!res.data.error) {
      ...
      getAllValues();
      document.getElementById("resets6").click();
      Swal.fire({
        title: "New Values Added!",
        icon: "success",
        confirmButtonText: "Okay",
        confirmButtonColor: "#da3c3b",
      });
  ...
  const onSubmit2 = async (data) => {
    await editValues(id, {
      ...
      attributeId: data?.edit_attribute,
      name_en: data?.edit_values_en?.trim(),
      name_ar: data?.edit_values_ar?.trim(),
    }).then((res) => {
      if (!res.data?.error) {
        document.getElementById("onClose4").click();
        getAllValues();
        Swal.fire({
        ...
        <div className="col-12 mb-4 inner_design_comman border">
          ...
          >
              ...
              {(formValues || [])?.map((item, index) => (
                <div className="form-group mb-0 row  rounded p-3 mb-2">
                  <div className="form-group mb-3 col-6">
                    <label for="">Enter Value Name (En)</label>
                    <input
                      type="text"
                      name="name_en"
                  ...
                  <div className="form-group mb-3 col-5">
                    <label for="">Enter Value Name (ar)</label>
                    <input
                      type="text"
                      name="name_ar"
                      lang="ar"
            ...
            <div className="form-group mb-0 col-12 d-flex justify-content-center">
              ...
              <button className="comman_btn2 d-none" type="reset" id="resets6">
                Save
              </button>
            </div>
          </form>
          ...
          <div className="row comman_header justify-content-between">
            ...
            <div class="col-auto">
              <input
                type="date"
                class="custom_date"
                onChange={(e) => getAllValues(e.target.value)}
              />
            </div>
          </div>
          <div className="row">
                  ...
                  <tbody>
                    {(AllValues || [])?.map((item, index) => (
                      <tr>
                        ...
                        <td>{item?.subSubCategory?.name_en}</td>
                        <td>{item?.name_en}</td>
                        <td>{item?.name_en}</td>
                        <td>{item?.name_ar}</td>
                        <td>
          ...
          <div className="modal-content">
            <div className="modal-header">
              ...
              <button
                ...
                aria-label="Close"
                id="onClose4"
                onClick={() => document?.getElementById("reset4").click()}
              ></button>
            </div>
            <div className="modal-body">
              ...
              >
                ...
                <div className="form-group col-6">
                  ...
                  <select
                    className={classNames("form-control form-select", {
                      "is-invalid": errors2.edit_attribute,
                    })}
                    {...register2("edit_attribute", {
                      required: "Category is required!",
                      onChange: (e) => {
                        SubCategory(e.target.value);
                      },
                    ...
                    name="edit_attribute"
                  >
                    <option value={selected?.attributeId} selected="">
                      {selected?.attribute}
                    </option>
                    {(Attributes || [])?.map((item, ind) => (
                      <option value={item?._id}>
                  ...
                  {errors2.edit_attribute && (
                    <small className="errorText mx-1">
                      *{errors2.edit_attribute?.message}
                    </small>
                  )}
                </div>
                <div className="form-group col-6">
                ...
                <div className="form-group col-6">
                  <label for="">Value Name (En)</label>
                  <input
                    type="text"
                    className={classNames("form-control", {
                      "is-invalid": errors2.edit_values_en,
                    })}
                    name="edit_values_en"
                    {...register2("edit_values_en", {
                      required: "Filter Name is required!",
                      pattern: {
                        value: /^[A-Za-z\s]{1,}[\.]{0,1}[A-Za-z\s]{0,}$/,
                        message: "Special Character not allowed!",
                  ...
                  {errors2.edit_values_en && (
                    <small className="errorText mx-1">
                      *{errors2.edit_values_en?.message}
                    </small>
                  )}
                </div>
                <div className="form-group col-6">
                  <label for="">Value Name (Ar)</label>
                  <input
                    type="text"
                    lang="ar"
                    dir="rtl"
                    className={classNames("form-control", {
                      "is-invalid": errors2.edit_values_ar,
                    })}
                    name="edit_values_ar"
                    {...register2("edit_values_ar", {
                  ...
                  />
                  {errors2.edit_values_ar && (
                    <small className="errorText mx-1">
                      *{errors2.edit_values_ar?.message}
                    </small>
                  )}
                </div>
